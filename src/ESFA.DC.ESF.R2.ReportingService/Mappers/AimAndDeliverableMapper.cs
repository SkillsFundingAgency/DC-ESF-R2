...
namespace ESFA.DC.ESF.R2.ReportingService.Mappers
{
    public sealed class AimAndDeliverableMapper : ClassMap<AimAndDeliverableModel>, IClassMapper
    {
        public AimAndDeliverableMapper()
        {
            ...
            Map(m => m.LearnStartDate).TypeConverterOption.Format("dd/MM/yyyy").Index(i++).Name("Learning start date");
            Map(m => m.LDESFEngagementStartDate).TypeConverterOption.Format("dd/MM/yyyy").Index(i++).Name("Learning start date of first assessment");
            Map(m => m.LearnPlanEndDate).TypeConverterOption.Format("dd/MM/yyyy").Index(i++).Name("Learning planned end date");
            Map(m => m.CompStatus).Index(i++).Name("Completion status");
            Map(m => m.LearnActEndDate).TypeConverterOption.Format("dd/MM/yyyy").Index(i++).Name("Learning actual end date");
            ...
            Map(m => m.LatestPossibleStartDate).TypeConverterOption.Format("dd/MM/yyyy").Index(i++).Name("Latest possible progression start date");
            Map(m => m.EligibleProgressionOutomeStartDate).TypeConverterOption.Format("dd/MM/yyyy").Index(i++).Name("Eligible outcome start date");
            Map(m => m.EligibleOutcomeEndDate).TypeConverterOption.Format("dd/MM/yyyy").Index(i++).Name("Eligible outcome end date");
            Map(m => m.EligibleOutcomeCollectionDate).TypeConverterOption.Format("dd/MM/yyyy").Index(i++).Name("Eligible outcome collection date");
            Map(m => m.EligibleOutcomeDateProgressionLength).TypeConverterOption.Format("dd/MM/yyyy").Index(i++).Name("Eligible outcome date used for progression length");
