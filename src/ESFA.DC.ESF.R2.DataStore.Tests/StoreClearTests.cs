...
namespace ESFA.DC.ESF.R2.DataStore.Tests
{
    public class StoreClearTests
    {
        ...
        public async Task StoreAsync()
        {
            ...
            var parameters = new object[] { ukprn, conRef };

            var datastoreQueryMock = new Mock<IDataStoreQueryExecutionService>();
            datastoreQueryMock.Setup(x => x.ExecuteStoredProcedure(DataStoreConstants.StoredProcedureNameConstants.DeleteExistingRecords, parameters, connection, It.IsAny<SqlTransaction>(), cancellationToken)).Returns(Task.CompletedTask);
